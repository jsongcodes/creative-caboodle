...
export default function extendTheme(options = {}, ...args) {
  ...
  const shape = _extends({}, input.sys?.shape, md3shape, {
    corner: _extends({}, input.sys?.shape?.corner, md3shape.corner)
  });
  const motion = createMotions(input.sys?.motion);
  const typescale = _extends({}, md3Typescale, input.sys?.typescale);
  const typeface = _extends({}, md3Typeface, input.ref?.typeface);
  const state = _extends({}, md3State, input.sys?.state);
  const _createThemeWithoutVa = createThemeWithoutVars(_extends({}, input, {
      ...
      sys: _extends({}, input.sys, {
        ...
        state,
        motion,
        color: _extends({}, md3LightColors, colorSchemesInput.light?.sys?.color),
        elevation: colorSchemesInput.light?.sys?.elevation ?? elevationLight,
        shape
      }),
      palette: _extends({}, colorSchemesInput.light && colorSchemesInput.light?.palette)
    })),
    {
  ...
  const {
    ...
    sys: _extends({}, input.sys, {
      ...
      state,
      motion,
      color: _extends({}, md3DarkColors, colorSchemesInput.dark?.sys?.color),
